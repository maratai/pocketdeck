# gptn.py - ChatGPT client with native function calling (tools) instead of the
# markdown code-block "agent mode" used by gpt.py.
#
# This reuses gpt.py for all the shared plumbing (api key, logging, STT/TTS,
# the thinking animation, message formatting, etc.) and adds:
  ...
  def ask_agent(self, message, references, images, model, instructions, effort,
    ...
    for i in range(max_iters + 1):
      ...
      for fc in fn_calls:
        ...
        print("\n%s[Call]%s %s %s" % (el.bold(), el.bold_off(), name, arguments), file=self.vs)
